...
if (ancombc_available) {
 ...
 # --- Model coefficients (interaction terms) ---
 ...
 write.csv(
 ancom_treatment,
 file.path(stats_dir, "ancombc2_interaction_terms.csv"),
 row.names = FALSE
 )
 }
...
# --- Per-timepoint ANCOM-BC2 (absolute treatment effects for volcano plots) ---
# Runs ANCOM-BC2 separately at each timepoint with fix_formula = "Treatment"
# This gives clean absolute treatment effects + q-values per timepoint
# (NOT interaction terms, which are differences vs reference level)
if (ancombc_available) {
 cat(" Running per-timepoint ANCOM-BC2 for volcano plots...\n")
 ancom_per_tp <- list()
 tax_info_ancom <- as.data.frame(tax_table(ps_genus)) %>%
 rownames_to_column("OTU") %>%
 dplyr::select(OTU, Genus, Family, Order, Phylum)

 for (f in mb_params$timepoints) {
 ps_tp <- subset_samples(ps_genus, Feces == f)
 ps_tp <- prune_taxa(taxa_sums(ps_tp) > 0, ps_tp)

 res_tp <- tryCatch({
 ancombc2(data = ps_tp, fix_formula = "Treatment",
 rand_formula = "(1 | Animal)", group = "Treatment",
 p_adj_method = mb_params$fdr_method,
 prv_cut = mb_params$prevalence_threshold,
 lib_cut = mb_params$min_reads)
 }, error = function(e) {
 cat(" ANCOM-BC2 failed for", f, ":", conditionMessage(e), "\n")
 NULL
 })

 if (!is.null(res_tp) && !is.null(res_tp$res)) {
 tp_df <- res_tp$res %>%
 dplyr::select(
 OTU = taxon,
 !!paste0("log2FC_", f) := lfc_TreatmentTAM,
 !!paste0("SE_", f) := se_TreatmentTAM,
 !!paste0("W_", f) := W_TreatmentTAM,
 !!paste0("p_", f) := p_TreatmentTAM,
 !!paste0("q_", f) := q_TreatmentTAM
 )
 ancom_per_tp[[f]] <- tp_df
 }
 }

 if (length(ancom_per_tp) > 0) {
 ancom_model_coef <- ancom_per_tp[[1]] %>%
 left_join(tax_info_ancom, by = "OTU")
 if (length(ancom_per_tp) > 1) {
 for (i in 2:length(ancom_per_tp)) {
 ancom_model_coef <- ancom_model_coef %>%
 left_join(ancom_per_tp[[i]], by = "OTU")
 }
 }
 write.csv(ancom_model_coef,
 file.path(stats_dir, "ancombc2_model_coefficients.csv"),
 row.names = FALSE)
 cat(" Per-timepoint ANCOM-BC2 saved to ancombc2_model_coefficients.csv\n")
 }
}
