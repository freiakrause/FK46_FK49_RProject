...
setwd("C:/Users/b1084855/OneDrive - Universität Salzburg/Freigegebene Dokumente - AG_Tumorimmunologie/Data/Freia Krause/01_Experiments/FK49_CD-HFD_13wks/FK49_Analysis")
  ...
  setwd("C:/Users/b1084855/OneDrive - Universität Salzburg/Freigegebene Dokumente - AG_Tumorimmunologie/Data/Freia Krause/01_Experiments/FK46_iALmice_high Fat diet 52 weeks 7d after injection/Analysis/")
    } else("Please Set Exp ID and setwd")
...
nash_summary <- data %>%
  filter(Sex == "male")%>%
  select("Animal","Sex","Treatment","BATCH", "NASH_B","NASH_S","NASH_I","NASH_S2")%>%
  filter(BATCH %in% c(1, 2)) %>%
  filter(complete.cases(NASH_S2)) %>%
  mutate(NASH_Category = case_when(
    is.na(NASH_S2) ~ NA_character_,
    NASH_S == "0" ~ "No NASH",
    NASH_S != "0" & NASH_B == "0" & NASH_I == "0" ~ "Only Steatosis",
    NASH_S != "0" & NASH_B != "0" & NASH_I == "0" ~ "NAFLD",
    NASH_S != "0" & NASH_B == "0" & NASH_I >"0"& NASH_I < "1" ~ "Borderline NASH",
    NASH_S != "0" & NASH_B != "0" & NASH_I >"0"& NASH_I < "1" ~ "Borderline NASH",
    NASH_S != "0" & NASH_B == "0" & NASH_I >= "1" ~ "Definite NASH",
    NASH_S != "0" & NASH_B != "0" & NASH_I >= "1" ~ "Definite NASH",
    TRUE ~ NA_character_)) %>%
  #filter(!is.na(NASH_Category)) %>%
  mutate(NASH_Category = factor(NASH_Category,levels = c("Definite NASH","Borderline NASH", "NAFLD","Only Steatosis","No NASH"))) %>%
...
#Combine all blocks
waffle_data <- bind_rows(ctrl_blocks, dummy_blocks, tam_blocks) %>%
  select(NASH_Category, fill_type, value) %>%
  filter(value > 0) %>%
  uncount(value)%>%mutate(fill_type=factor(fill_type, levels=c("ctrl","TAM","dummy")))  # one row per tile
...
nash_p <- ggplot(waffle_data, aes(fill = fill_type, values = 1)) +
  ...
  scale_fill_manual(values = c("ctrl" = "#B0B0B0", "TAM" =  "#C05050", "dummy" = "white"),
                    name = "Treatment", labels = c("ctrl"="Control","TAM"="TAM","dummy"="")) +
...
ggsave(filename = paste0(ExpId,"_NASHScore_male.png"), plot = nash_p, path = "02_GeneratedData/NASH_Score", width = 10, height =12 ,dpi = 300)
...
df_long <- NASH_Fishers %>%
  ...
  uncount(count)%>%
  mutate(NASH_Category=factor(NASH_Category,levels=c("No NASH","Only Steatosis","NAFLD","Borderline NASH","Definite NASH")) )              # replicate rows according to count
...
p1<-ggbarstats( df_long, NASH_Category, Treatment,results.subtitle = FALSE, subtitle = paste0("Fisher's exact test", ", p-value = ",round(test$p.value, 5)))+
  scale_fill_manual(values = c("No NASH" ="#A8E6B1", "Only Steatosis" =  "#FFDAB9","NAFLD" = "#FDBB10", "Borderline NASH" = "#FDBA74","Definite NASH" = "#FF9999"))

ggsave(filename = paste0(ExpId,"_NASHScore_bar_both.png"), plot = p1, path = "02_GeneratedData/NASH_Score", width = 5, height =10 ,dpi = 300)
...
for (i in vars) {
   plots[[i]] <- ggplot(d, aes(x = Treatment, y = .data[[i]], fill = Treatment)) +
    geom_boxplot() +
    scale_fill_manual(values = c("ctrl" = "#B0B0B0", "TAM" = "#C05050")) +
...
## Preapre Matrix and Data for HeatMap -----
  ...
  wks <- NULL
if (ExpId == "FK49"){wks <- "13 wks"
}else if (ExpId == "FK46"){wks <- "long-term"
  }else{wks <- ""}  
## Heatmap  -----
  h <- pheatmap(mat,
    cluster_rows = FALSE,
    ...
    annotation_colors = list(Treatment = c("ctrl" = "#B0B0B0", "TAM" = "#C05050")),
    main =paste0("NASH Scoring Heatmap iAL after ",wks," CD-HFD"),
    labels_col = "Animals",
